...
if [ "$BINARY_CHANGED" = true ]; then
    ...
    if [ -d "$APP_BUNDLE/Contents/Frameworks/Sparkle.framework" ]; then
        codesign --force --sign - "$APP_BUNDLE/Contents/Frameworks/Sparkle.framework" 2>&1 || echo "⚠️  Sparkle signing failed"
    fi
    codesign --force --sign - "$APP_BUNDLE" 2>&1 || echo "⚠️  App signing failed"
